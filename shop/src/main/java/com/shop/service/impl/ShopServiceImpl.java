...
public class ShopServiceImpl extends BaseServiceImpl implements ShopService {
	...
	public List<Map<String, Object>> UserlistPopularShop() {
		...
		List<Map<String, Object>> result = this
				.find("SQM:select distinct s.shop_id as shopId,"
	...
	public List<Map<String, Object>> getShopPic(int shop_id) {
		...
		List<Map<String,Object>> result = this.find("SQM:select p.pic_url as picUrl,p.pic_type as picType "
				...
				+ "or (s.shop_id = :shop_id and p.pic_type = 'shopBackground')", params);	
	...
	public Number getShopTotalProdNum(int shop_id) {
		...
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("shopId", shop_id);
		List list = this.find("Select count(p.prodId) as number from Product p where p.shop.shopId = :shopId",params);
